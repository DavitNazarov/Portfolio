...
import { locationLine, metaRows, wrapShell } from "./shared.js";
...
export function buildVisitEmail(meta: VisitorMeta): EmailOptions {
  ...
  const location = locationLine(meta);
  const text = [
    "Someone just opened your portfolio.",
    "",
    `When: ${new Date().toISOString()}`,
    location && `Location: ${location}`,
    meta.coordinates && `Coordinates: ${meta.coordinates}`,
    meta.timezone && `Timezone: ${meta.timezone}`,
    meta.org && `Network: ${meta.org}`,
    meta.ip && `IP: ${meta.ip}`,
